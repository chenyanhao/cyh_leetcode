package p201_300.p201数字范围按位与;

/**
 * 以 left=9, right=12 为例，
 *
 * 9        0 0 0 0 1 0 0 1
 * 10       0 0 0 0 1 0 1 0
 * 11       0 0 0 0 1 0 1 1
 * 12       0 0 0 0 1 1 0 0
 * result   0 0 0 0 1 0 0 0
 *
 * 考虑到按位与运算的性质，对于一系列的位，只要有一个零值的位，那么这一系列位的按位与运算结果都将为零。
 * 从例子中可以发现，对所有数字执行按位与运算的结果，是所有对应二进制字符串的公共前缀，再用零补上后面的剩余位。
 * 所以问题转化为：给定两个整数，找到它们对应的二进制字符串的公共前缀。
 *
 * 解法一：
 * 1）将两个数字不断向右移动，直到数字相等，即数字被缩减为它们的公共前缀。
 * 2）通过将公共前缀向左移动，将零添加到公共前缀的右边以获得最终结果。
 *
 * 还是以 left=9, right=12 为例，
 * 1) 初始
 *  left=9     0 0 0 0 1 0 0 1
 *  right=12   0 0 0 0 1 1 0 0
 * 2) 右移一位
 *  left=4     0 0 0 0 0 1 0 0
 *  right=6    0 0 0 0 0 1 1 0
 * 3) 继续右移
 *  left=2     0 0 0 0 0 0 1 0
 *  right=3    0 0 0 0 0 0 1 1
 * 4) 继续右移
 *  left=1     0 0 0 0 0 0 0 1
 *  right=1    0 0 0 0 0 0 0 1
 * 5) 此时 left == right，停止移动。公共前缀为 0 0 0 0 0 0 0 1。
 *  因为一共右移了 3 次，所以将公共前缀左移 3 位，得到最终结果 0 0 0 0 1 0 0 0
 *
 *
 * 时间复杂度：O(log_right)。因为 left < right，时间复杂度取决于 right 的二进制位数。
 * 空间复杂度：O(1)。只需要常数空间。
 *
 */
class Solution {
    public int rangeBitwiseAnd(int left, int right) {
        int shiftCount = 0;
        // 找公共前缀
        while (left < right) {
            left = left >> 1;
            right = right >> 1;
            ++shiftCount;
        }
        return left << shiftCount;
    }
}
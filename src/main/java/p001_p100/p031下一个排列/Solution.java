 ...
 * 2）下一个数增加的幅度尽可能的小。
 *
 * 对于第一点，只需要将后面的「大数」与前面的「小数」交换，就能得到一个更大的数。例如123，交换3和2就能得到一个更大的数 132。
 * 对于第二点，需要「大数」尽可能小、而「小数」尽可能大，也就是在数组索引上看，「小数」的索引尽可能小、「大数」索引尽可能大。这样可以保证，
 * 1）在尽可能靠右的低位进行交换，因此需要从后向前查找
 * 2）找到的满足条件的「大数」要尽可能的小，例如123465，应该交换5和4，而不是6和4交换，即大数是5而不是6
